...
import openpi.models.pi0_fast_hooked as pi0_fast_hooked
...
_CONFIGS = [
    ...
    TrainConfig(
        name='pi0_fast_hooked',
        batch_size=1,
        model=pi0_fast_hooked.Pi0FASTConfig(action_dim=8, action_horizon=10),
        data=SimpleDataConfig(
            assets=AssetsConfig(asset_id="droid"),
            data_transforms=lambda model: _transforms.Group(
                inputs=[droid_policy.DroidInputs(action_dim=model.action_dim, model_type=ModelType.PI0_FAST)],
        ...
        policy_metadata={'hooked': True},
    ),
    TrainConfig(
        name='pi0_fast_not_hooked',
        batch_size=1,
        model=pi0_fast.Pi0FASTConfig(action_dim=8, action_horizon=10),
        data=SimpleDataConfig(
            assets=AssetsConfig(asset_id="droid"),
    ...
    TrainConfig(
        name="pi0_fast_ur5",
        model=pi0_fast.Pi0FASTConfig(action_dim=32, action_horizon=32), # TODO pi0_fast.Pi0FASTConfig(action_dim=8, action_horizon=10),
        data=LeRobotUR5DataConfig(
            repo_id="iantc104/ur5",
    ...
    TrainConfig(
        name="pi0_fast_ur5_low_mem_finetune",
        # Here is an example of loading a pi0-FAST model for LoRA finetuning.
        # For setting action_dim, action_horizon, and max_token_len, see the comments above.
        model=pi0_fast.Pi0FASTConfig(
            action_dim=32, action_horizon=20, max_token_len=250, paligemma_variant="gemma_2b_lora"
        ),
        data=LeRobotUR5DataConfig(
            repo_id="iantc104/seal_ur5",
            # This config lets us reload the UR5 normalization stats from the base model checkpoint.
            # Reloading normalization stats can help transfer pre-trained models to new environments.
            # See the [norm_stats.md](../docs/norm_stats.md) file for more details.
            assets=AssetsConfig(
