...
def callback_func():
    return jnp.load('/home/kaylene/temp_gemma.npy')
...
class Block(nn.Module):
    ...
    def setup(self):
        ...
        self.mlp = lora.FeedForwardHooked(
    ...
    def __call__(self, x, kv_cache, positions, attn_mask, hyperactivate_mask, hyperactivate_coeff, decode, deterministic=True):  # noqa: FBT002
        ...
        outputs = self.mlp(attn_output, hyperactivate_mask, hyperactivate_coeff)
    ...
    def __call__(
        ...
        blocks = [
            nn.scan(
                ...
                in_axes=(0, nn.broadcast, nn.broadcast, 0, nn.broadcast, nn.broadcast, nn.broadcast),  # 0=kv_cache, 1=positions, 2=mask
                length=self.depth,
            )(parent=layers, **block_kw)
        ]
        # hyperactivate_mask should be a jnp array where the first dimension is the layer index
        # and second dimension is the indexes of the value vectors within each layer to hyperactivate
        for block in blocks:
            x, kv_cache = block(x, kv_cache, positions, mask, hyperactivate_mask, hyperactivate_coeff, decode, deterministic)

        # jax.debug.breakpoint()
        # jax.debug.print('x before intervention: {}', x[0,0,:10])
        # x = callback_func()
        # jax.debug.print('x after intervention: {}', x[0,0,:10])
