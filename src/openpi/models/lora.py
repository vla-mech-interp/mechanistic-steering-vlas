...
class FeedForwardHooked(FeedForward):
    @nn.compact
    def __call__(self, x, hyperactivate_mask, hyperactivate_coeff=10.):
        dtype = x.dtype  # original dtype, could be half-precision
        ff_gate = self._dot(
            x,
            self.w_gating[0],
        ...
        if hyperactivate_mask is not None:
            activations = jnp.where(hyperactivate_mask, hyperactivate_coeff, activations)
